...
export default function DefaultDetailPage() {
  ...
  // Разделение агентов по сторонам
  ...
  const handleCommentUpdate = (stepId: number, newComment: string) => {
    if (!selectedStep) return;
    ...
    <div className="px-4 sm:px-6 lg:px-16 py-4">
      <div className="grid grid-cols-1 md:grid-cols-[minmax(260px,1fr)_2fr_minmax(260px,1fr)] gap-4 md:gap-6 lg:gap-8">
        <StepPanel
          ...
          teamName={defaultData.team.name}
          mapName={defaultData.map.name}
          side={defaultData.side}
          roundNumber={defaultData.roundNumber}
          opponentTeamName={defaultData.opponentTeamName}
        ...
        <MapColumn
          ...
          onAgentDropOnTrash={agentActions.handleAgentDropOnTrash}
          agents={agents}
          onSelectAgent={agentActions.handleSelectAgent}
          onAddAgent={agentActions.handleAddAgent}
        />
        <RightPanel
          stepNumber={selectedStep?.stepNumber}
          ...
          onAbilityAdded={abilityActions.handleAbilityAdded}
          onAbilityDeleted={abilityActions.handleAbilityDeleted}
          onAbilityUpdated={handleAbilityUpdated}   // <-- добавьте эту строку
          onCreateAgent={agentActions.handleCreateAgent}
          onCommentUpdate={handleCommentUpdate}
        />
      </div>
